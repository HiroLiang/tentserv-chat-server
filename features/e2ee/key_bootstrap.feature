Feature: E2EE key bootstrap after login
  ...
  Scenario: Device-scoped keys remain isolated across multiple login devices
    ...
    Then the response status should be 200
    When I upload E2EE signed pre-key "beta" with key id 2 for device "22222222-2222-2222-2222-222222222222"
    Then the response status should be 204
    When I upload 4 E2EE one-time pre-keys starting at key id 10 for device "22222222-2222-2222-2222-222222222222"
    Then the response status should be 200
    When I request E2EE key status for the logged in user and device "22222222-2222-2222-2222-222222222222"
    Then the response status should be 200
    And E2EE key status should expose identity "beta", signed pre-key "beta", key id 2, and 4 OTP keys for device "22222222-2222-2222-2222-222222222222"
    When I login with identifier "e2ee-login@example.com", password "correct-password", and device "11111111-1111-1111-1111-111111111111"
    Then the response status should be 200
    And login response should include a bearer token
    When I request E2EE key status for the logged in user and device "11111111-1111-1111-1111-111111111111"
    Then the response status should be 200
    And E2EE key status should expose identity "alpha", signed pre-key "alpha", key id 1, and 2 OTP keys for device "11111111-1111-1111-1111-111111111111"
